...
def _grab_tile(lat, lon, zoom, maptype, _TILESIZE, sleeptime):
    global File_JPG
    '''https://maps.googleapis.com/maps/api/staticmap?center=1.32136,103.86038&zoom=16&size=640x400&path=weight:3%7Ccolor:blue%7Cenc:{coaHnetiVjM??_SkM??~R&key=AIzaSyDN_41GsxnkcD676XX9nKqqYIfcI3jezfc'''
    ...
    urlbase += '&size=%dx%d'
    urlbase += '&format=jpg'
    urlbase += '&markers=color:blue|label:BIKE|%f,%f'
    urlbase += '&key='
    urlbase += _KEY

    specs = lat, lon, zoom, maptype, 640, 480,lat, lon
    file_data = lat, lon, zoom, maptype, 640, 480

    filename = './mapscache/' + ('%f_%f_%d_%s_%d_%d' % file_data) + '.jpg'
    File_JPG =  ('%f_%f_%d_%s_%d_%d' % file_data) + '.jpg'
    tile = None
...
def fetchTiles(latitude, longitude, zoom, maptype, radius_meters=None, default_ntiles=4):
    ...
    bigimage = _new_image(640, 480)


    tile = _grab_tile(latitude, longitude, zoom, maptype, _TILESIZE, 1. / _GRABRATE)
...
class GooMPy(object):
    ...
    def getFileName(self):
        ...
        Returns the current image as a PIL.Image object.
        '''
        self.File_JPG = File_JPG

        return self.File_JPG
    ...
    def _update(self):
        self.winimage.paste(self.bigimage)
